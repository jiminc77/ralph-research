...
\newcommand{\resultsummary}{Under the preregistered labeling rule, t1a straighten is labeled \emph{contact-dominant}; t1b single bend is labeled \emph{balanced/inconclusive}; t1c endpoint reposition is labeled \emph{direction-dominant} on this pilot.}
...
\newcommand{\interactionsummary}{Interaction estimates are t1a straighten: +0.228 [+0.217, +0.239]; t1b single bend: +0.154 [+0.138, +0.168]; t1c endpoint reposition: +0.007 [-0.007, +0.020]. All three exceed the zero-displacement noise floor.}
...
\newcommand{\guardlead}{The invalid-table rate guard fails: 42 of 192 tables are invalid (rate 0.219 $>$ 0.02), dominated by 6 states with a canonicalization re-placement artifact (error up to $6.4{\times}10^{-3}$); the registered 0.02 threshold admits less than one invalid table per candidate at this pilot scale, so under the strict fixed-priority rule no candidate regime is selectable and all estimates in this paper are descriptive. Invalid tables are quarantined, never averaged in, and listed in the released data. The full-state-set no-op p99 guard fails, driven entirely by state(s) whose tables are quarantined; restricted to used states the no-op drift is median $1.0{\times}10^{-8}$, p99 $1.5{\times}10^{-4}$, well within thresholds.}
\newcommand{\absguards}{Repeatability, drift, and noise-floor controls pass on all used tables, but a canonicalization re-placement artifact invalidates 42/192 state tables (quarantined and reported separately) and makes the strict preregistered invalid-rate guard unsatisfiable at pilot scale, so no candidate regime is formally selectable and all numbers are descriptive.}
